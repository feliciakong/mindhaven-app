...
import { Sparkles, Shield, Lock, Brain, Compass, Feather, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-stone-50 via-amber-50/20 to-emerald-50/30 dark:from-[#fcfbf7] dark:via-stone-900 dark:to-emerald-950/20 text-stone-800 dark:text-stone-900 flex flex-col justify-between transition-colors duration-300">
      ...
      <main className="w-full max-w-5xl mx-auto px-6 py-12 md:py-20 flex flex-col items-center text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-100/80 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-medium mb-8 backdrop-blur-sm">
          <Sparkles className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
          <span>Powered by Gemini 3.6 Flash & Firebase Firestore</span>
        </div>

        <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold text-stone-900 dark:text-stone-50 tracking-tight leading-tight max-w-3xl">
        ...
        {authError && (
          <div className="mt-6 p-4 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 text-sm max-w-md text-center">
            {authError}
          </div>
        )}
      ...
      <footer className="w-full border-t border-stone-200/60 dark:border-stone-200 py-6 text-center text-xs text-stone-500 dark:text-stone-600">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span>End-to-end user isolation • No password storage</span>
          </div>
          <div>
            <span>MindHaven © {new Date().getFullYear()} • Personal AI Reflection Journal</span>
          </div>
        </div>
      </footer>
    </div>
